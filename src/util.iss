...
function GetUserHive(): Integer; forward;
function GetEnvironmentKey(): String; forward;
...
function IsHttpProxyEnv(): Boolean;
var
  RootKey: Integer;
  SubKey: String;
begin
  ...
  if RegValueExists(RootKey, SubKey, 'HTTP_PROXY') then
    Result := True
  else
    Result := False;
end;

function IsProxyServer(): Boolean;
var
  RootKey: Integer;
  SubKey: String;
  ProxyEnableFlag: Cardinal;
begin
  RootKey := HKCU;
  SubKey := 'Software\Microsoft\windows\CurrentVersion\Internet Settings';

  RegQueryDWordValue(RootKey, SubKey, 'ProxyEnable', ProxyEnableFlag);

  if ProxyEnableFlag = 1 then
  begin
    if RegValueExists(RootKey, SubKey, 'ProxyServer') then
      Result := True
    else
      Result := False;
  end else
...
function GetHttpProxyEnv(): String;
var
  RootKey: Integer;
  SubKey, Proxy: String;
begin
  ...
  RegQueryStringValue(RootKey, SubKey, 'HTTP_PROXY', Proxy);
...
var
  Proxy: String;
begin
  RegQueryStringValue(HKCU, 'Software\Microsoft\windows\CurrentVersion\Internet Settings', 'ProxyServer', Proxy);
